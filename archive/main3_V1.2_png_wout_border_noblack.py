...
def process_image(file_path, output_folder, n_colors=5):
    ...
    dominant = filtered_palette[np.argmax(filtered_counts)]

    avg_patch = np.ones(shape=img.shape, dtype=np.uint8)*np.uint8(average)

    freqs = np.cumsum(np.hstack([[0], filtered_counts/float(filtered_counts.sum())]))
    rows = np.int_(img.shape[0]*freqs)

    dom_patch = np.zeros(shape=img.shape, dtype=np.uint8)
    for i in range(len(rows) - 1):
        dom_patch[rows[i]:rows[i + 1], :, :] += np.uint8(filtered_palette[i])
    ...
    with open(output_csv_file, mode='a', newline='') as file:
        writer = csv.writer(file)
        for color, count in zip(filtered_palette, filtered_counts):
