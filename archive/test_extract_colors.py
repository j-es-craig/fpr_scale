...
if not os.path.exists(output_csv_file):
    with open(output_csv_file, mode='w', newline='') as file:
        ...
        writer.writerow(['FileName', 'R', 'G', 'B', 'Count', 'Brightness', 'Whiteness'])
...
def save_dominant_colors_to_csv(file_name, top_colors, top_counts):
    ...
    with open(output_csv_file, mode='a', newline='') as file:
        ...
        for color, count in zip(top_colors, top_counts):
            ...
            whiteness = calculate_whiteness(color)
            writer.writerow([file_name] + list(color) + [count, brightness, whiteness])
...
def calculate_whiteness(color):
    """Calculate the whiteness of a given color."""
    return np.sqrt((255 - color[0])**2 + (255 - color[1])**2 + (255 - color[2])**2)
